...
public class Get17 extends DummyRestApiBaseUrl {
    ...
    public void name() {
        ...
        DummyApiDataPojo dataPojo = new DummyApiDataPojo(1, "Tiger Nixon", 32000, 61, "");
        ResponseBodyPojo responseBodyPojo = new ResponseBodyPojo("success", dataPojo, "Successfully! Record has been fetched.");


        //3- send get request and get response
        Response response = given().spec(spec).when().get("/{first}/{second}");
        response.prettyPrint();

        //4- do assertions
        ResponseBodyPojo responseBody = JsonUtil.convertJsonToJavaObject(response.asString(), ResponseBodyPojo.class);
